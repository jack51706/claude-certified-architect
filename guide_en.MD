...
| Time limit | 120 minutes (as consistently reported; not stated in the official guide) |
| Scenarios | 4 randomly selected from the official set of **6** (see note below) |

> **The certification family (July 2026):** this guide covers **Claude Certified Architect — Foundations** ($125, open to all Claude Partner Network members; CPN membership is free). Three sibling certifications are open for registration via the Anthropic Partner Academy: **Associate — Foundations** ($99) for client-facing and delivery practitioners who use Claude in their own work (prompting, evaluating output, workflow integration, responsible use); **Developer — Foundations** ($125) for engineers who build and ship production applications and agents on Claude (custom tools, MCP servers, model optimization, security); and **Architect — Professional** ($175) for experienced architects leading enterprise-scale deployments (solution and integration architecture, optimization at scale, governance, stakeholder leadership).
...
> **Note on the official list:** the official exam guide defines **six** scenarios (its titles differ slightly: "Customer Support Resolution Agent", "Developer Productivity with Claude"), and each sitting draws 4 of those 6. Scenarios 7 and 8 below are **additional practice areas** this guide adds on top of the official six — they exercise the same five domains and are worth studying, but they are not named in the official scenario list.
...
**Enterprise tier: server-managed settings (above all three).** For Claude for Teams/Enterprise, an org **Owner** can push a fourth, highest-priority layer from the claude.ai console (**Admin Settings → Claude Code → Managed settings**) — no device-management (MDM) infrastructure required; clients fetch it at authentication and re-poll hourly. **This managed tier sits above every local file *and even command-line flags* — nothing a developer sets can override it.** It carries the same keys as `settings.json` — a `permissions.deny` list, `disableBypassPermissionsMode`, `allowManagedPermissionRulesOnly` (lock permissions to only the managed set), env vars, and hooks — so the guardrails below become **org-wide policy** instead of per-repo config. A companion admin control sets the **organization- (or per-role-) default model**: the `/model` picker's *Default* row then resolves to it, labelled *Org default* — but this one is a *soft* default that `--model` and `ANTHROPIC_MODEL` still override, not a lock (requires Claude Code **v2.1.196+**). On Bedrock/Vertex/Foundry, where server delivery isn't available, a self-hosted **Claude apps gateway** performs the equivalent delivery. **Caveat for the exam:** managed settings are a *client-side* control, not a security boundary — a modified binary, an older client, or a third-party provider bypasses them. Source: [server-managed settings](https://code.claude.com/docs/en/server-managed-settings), [model configuration](https://code.claude.com/docs/en/model-config).
...
flowchart TD
    ...
    B -->|Opus 4.7 / 4.8 / Fable 5 / Sonnet 5| C[thinking type adaptive<br/>budget_tokens rejected with 400]
...
§20.6 then builds a complete **compliance-research assistant** that mixes server and client tools in a single turn, and §20.7 distils the engineering judgement.
...
**Client tools** — both your own custom tools *and* Anthropic-schema tools like `bash` and `text_editor` — produce a `tool_use` block and `stop_reason: "tool_use"`. The turn **stops**, control returns to you, your code runs the operation, and you send a `tool_result` back on the next request. You own execution, the runtime, the network, and the blast radius.
...
|---|---|---|
| Web search | `web_search_20250305` (basic), `web_search_20260209`+ (dynamic filtering) | Live web search with **citations**; results carry `encrypted_content` that must be echoed back in multi-turn conversations. |
| Web fetch | `web_fetch_20250910` (basic), `web_fetch_20260209`+ | Retrieve/parse a specific URL or PDF (typically one already in the conversation). |
| Code execution | `code_execution_20250825` (+ `_20260120`, `_20260521`) | Sandboxed **Python + bash** container; data-science libraries preinstalled; containers reusable ~30 days. |
| Tool search | `tool_search_tool_regex` / `tool_search_tool_bm25` | Discover tools on demand from a large library; mark the rest `defer_loading: true` so their schemas are *appended* later (preserving the prompt cache). |
| Advisor | `advisor_20260301` (beta) | Pairs a cheaper executor model with a smarter advisor consulted mid-generation (advisor must be ≥ executor in capability). |
...
- **`tool_search` exists for scale, not features.** If an agent has access to hundreds of tools, putting every schema in the prompt is expensive and dilutes attention. Defer most of them and let Claude search for the few it needs; the discovered schemas are appended, so earlier cache breakpoints survive (Chapter 18).
...
```mermaid
flowchart TD
    Req[messages.create with server and client tools] --> Resp[Assistant response]
    Resp --> Q{stop_reason?}
    Q -->|end_turn| Done[Done — server tool ran inline]
    Q -->|pause_turn| Re[Re-send response as-is<br/>same tools array]
    Re --> Req
    Q -->|tool_use| Mix[Client tool_use present<br/>plus server_tool_use with no result]
    Mix --> Exec[Run YOUR client tool]
    Exec --> Only[Send user message of<br/>ONLY tool_result blocks]
    Only --> Req
```
...
| Tool | Meter | Notes |
|---|---|---|
| Web search | **$10 per 1,000 searches** | Each search = one use regardless of results. Errors are **not** billed. Citation `cited_text`/`title`/`url` don't count as tokens; result content does. |
...
- **Domain filtering is your network ACL for the web.** `allowed_domains` / `blocked_domains` (one or the other, never both) gate which sites Claude can reach. No scheme, subdomains auto-included, no `*.` wildcard in the host. **Use ASCII-only domains** — a Cyrillic `а` in `аmazon.com` is a homograph attack that slips past a naïve allow-list.
...
flowchart TD
    ...
    App -->|tools array| Claude[Claude turn]
    Claude --> WS[Server tool web_search<br/>allowed_domains regulators only]
    Claude --> CE[Server tool code_execution<br/>risk score in sandbox]
    Claude --> DB[Client tool query_internal_db]
    WS -. citations + encrypted_content .-> Claude
    CE -. computed score .-> Claude
    DB --> Handler[Firm-hosted handler<br/>inside the network]
    Handler -. tool_result exposure .-> Claude
    Claude --> Answer[Cited risk briefing]
```
...
| Concept | What to remember |
|---|---|
| Two execution models | Client tools → `tool_use`, *you* execute and return `tool_result`; server tools → `server_tool_use` (`srvtoolu_`), Anthropic executes inside the turn, **no `tool_result`** (§20.1). |
| Server catalogue | Web search, web fetch, code execution, tool search, advisor — declared in `tools`, no infra to host (§20.2). |
| Programmatic tool calling | Mark a client tool `allowed_callers: ["code_execution_20260120"]` so Claude calls it from sandboxed code — many calls, filtered in-container, one reduced result to context (~24% fewer input tokens). Still a paused `tool_use`: you execute and return `tool_result`; not a security boundary, not ZDR (§20.2). |
| Anthropic-schema client tools | `bash`, `text_editor`, `memory`, computer use ship schemas but **you execute** them — sandbox accordingly (§20.3). |
| `pause_turn` vs mixed `tool_use` | Long server loop → `pause_turn`, re-send as-is (same tools). Server **+ client** in one turn → `tool_use`; reply with **only** `tool_result` blocks (§20.4). |
| Cost meters | Web search **$10/1k** (bound with `max_uses`); code execution **free with web tools**, else **$0.05/hr/container** after 1,550 free hrs (§20.5). |
| Security boundary | Code-execution sandbox has **internet disabled**; ZDR excludes dynamic-filtering web tools unless `allowed_callers: ["direct"]`; keep sovereign data on **client** tools; ASCII-only domain allow-lists (§20.5). |
...
§21.1 frames the budget and the 200K-vs-1M decision; §21.5 covers the interactions that bite in production (prompt caching, what to keep vs drop); §21.6 builds a complete **multi-day codebase-migration agent** end-to-end; §21.7 distills the decision rules.
...
| Claude Haiku 4.5 | **200K tokens** | The cheap, fast tier — and the one you hit *first* |

**The trap is treating a bigger window as a fix.** It is not, for three reasons an architect must internalize:

- **Recall degrades with length (context rot).** Attention relates every token pair — an n² cost that stretches thin over long inputs. A fact at 600K tokens is *attended to* worse than the same fact at 6K. A bigger budget *delays* failure; it does not repeal it.
...
```mermaid
flowchart TD
    Budget[Context window budget<br/>finite even at 1M] --> Fixed[tools plus system<br/>stable prefix]
...
Compaction is server-side, lossy summarization that triggers automatically as the conversation approaches a threshold (default ~150K tokens). The API condenses earlier turns into a **compaction block**, then continues from that summary instead of the full history. You opt in per request.
...
```python
client.beta.messages.create(
    betas=["context-management-2025-06-27"],            # different beta header from compaction
    model="claude-opus-4-8",
    max_tokens=16000,
    tools=tools,
    messages=messages,
...
response = client.messages.create(
    model="claude-opus-4-8",
    max_tokens=16000,
    tools=[{"type": "memory_20250818", "name": "memory"}],   # schema-less, Anthropic-defined
...
# and returns tool_result. The SDKs ship BetaAbstractMemoryTool / betaMemoryTool helpers.
```
...
- **Compaction and context editing rewrite the `messages` prefix**, which invalidates cache reads from the edited point onward. That is usually an acceptable trade — you spend one cold prefill to reclaim tens of thousands of tokens every subsequent turn — but it means you should not trigger editing more aggressively than you need to.
...
def migration_turn(messages):
    return client.beta.messages.create(
        ...
        max_tokens=32000,
        tools=[
            {"type": "memory_20250818", "name": "memory"},        # durable, cross-session
            {"type": "text_editor_20250728", "name": "str_replace_based_edit_tool"},
            {"type": "bash_20250124", "name": "bash"},            # run tests
        ],
        context_management={"edits": [
            {"type": "compact_20260112"},                          # summarize when near limit
            {"type": "clear_tool_uses_20250919", "clear_tool_inputs": True},  # drop file/test dumps
        ]},
        messages=messages,
    )
...
| Concept | What to remember |
|---|---|
| Three distinct mechanisms | Compaction *summarizes* (in-session, lossy); context editing *clears* (in-session, lossless for survivors); memory *persists* (cross-session). Don't confuse them (§21.0–§21.4). |
| Compaction | Beta `compact-2026-01-12`, `compact_20260112`; auto-triggers near ~150K. **Append the whole `response.content` (compaction block included) every turn** or you lose the compacted state (§21.2). |
| Context editing | Beta `context-management-2025-06-27`; `clear_tool_uses_20250919` (+`clear_tool_inputs`) and `clear_thinking_20251015`. Different header/types from compaction (§21.3). |
| Memory tool | `memory_20250818`, client-side, `/memories`; survives sessions. **No secrets; scope per user; don't let it bloat** (§21.4). |
...
**Decision rule:** one request → base64 (simplest). Same asset reused across turns or requests → Files API (`file_id`). Public URL you trust → `url`. On **Amazon Bedrock and Google Vertex AI, only base64 is available** — the Files API and `url` sources are not, so a Bedrock/Vertex document agent has no choice but to inline bytes (and must budget for the repeated payload).
...
For any workload where a wrong-but-confident answer has consequences — legal, financial, compliance — a plain text answer is a liability. **Citations** make the model anchor each claim to an exact span of the source document.
...
flowchart TD
    ...
    Up --> Cnt[count_tokens<br/>budget per document]
    Cnt --> Extract[Pass 1 extract fields<br/>structured outputs schema]
    Up --> Verify[Pass 2 verify rates<br/>citations enabled]
    Extract --> Merge[Merge fields plus cited findings]
    Verify --> Merge
...
|---|---|
| Content blocks | `content` is an ordered list; `image` (JPEG/PNG/GIF/WebP) and `document` (PDF text + layout) carry non-text input (§22.1). |
| Source types | base64 (one-shot), `url` (hosted), `file_id` (reused across turns/requests). Bedrock/Vertex = base64 only (§22.2). |
| The base64 trap | base64 bytes re-send on every turn; switch reused assets to the Files API (`file_id`) to stop the bloat (§22.2). |
| Prompt order | Put documents/images **before** the text question; label multiple inputs `Image 1`, `Image 2` (§22.3). |
| Citations | `citations: {enabled: true}` → `cited_text` + page/char location; all-or-nothing; **incompatible with structured outputs** (§22.4). |
| Image cost | `ceil(w/28) × ceil(h/28)` visual tokens; high-res tier (Opus 4.7+) up to 4784 vs 1568 standard — downsample if fidelity isn't needed (§22.5). |
| PDF cost | Each page bills **text (1,500–3,000) + image tokens**; 600-page / 32 MB limit (100 pages on 200K models) (§22.5). |
| Token counting | Use `count_tokens` for an exact, model-specific count; never `tiktoken` (wrong for Claude, blind to visual tokens) (§22.5). |
...
**The message stream is the contract.** Whichever entrypoint you use, the SDK yields the same five core message types, and reading them correctly is what separates a robust integration from a fragile one:

| Message | When it fires | What you do with it |
|---|---|---|
| `SystemMessage` (`subtype="init"`) | First message of the session | Capture `session_id` (Python: `message.data["session_id"]`; TS: `message.session_id`). |
| `AssistantMessage` | After each Claude response | Progress: text + which tools were requested this turn. |
| `UserMessage` | After each tool execution | The tool result fed back to Claude (also your own streamed inputs). |
| `StreamEvent` | Only with partial messages enabled | Token-level deltas for live UIs. |
| `ResultMessage` | End of the loop | The terminal record: `subtype`, `result`, `total_cost_usd`, `usage`, `session_id`. |
...
A **session** is the conversation history the SDK accumulates while the agent works: your prompt, every tool call, every tool result, and every response. **The SDK writes it to disk automatically** — in Python always, in TypeScript unless you set `persistSession: false`. Returning to a session restores full context: files already read, analysis already done, decisions already made. (Sessions persist the *conversation*, not the filesystem — to snapshot and revert file changes, use file checkpointing.)
...
options = ClaudeAgentOptions(
    ...
    allowed_tools=["mcp__billing__refund"],   # note the mcp__<server>__<tool> naming
)
```
...
flowchart TD
    ...
    Allow --> Hook{{PreToolUse hook<br/>block main push and CI files}}
    Hook --> Loop[Built-in agent loop<br/>reproduce, locate, fix]
    Loop --> Sub[Agent tool delegates<br/>log-analysis subagent]
    Loop --> Budget{{max_budget_usd<br/>secondary guardrail}}
    Budget --> Res[ResultMessage<br/>branch the subtype]
...
options = ClaudeAgentOptions(
    ...
    setting_sources=["project"],        # load CLAUDE.md conventions; survives compaction
    agents={                            # least-privilege subagent for noisy log analysis
...
| Concept | What to remember |
|---|---|
| Two entrypoints | `query()` for one-shot async-iterator tasks; `ClaudeSDKClient` (Python) to hold a multi-turn session. TS uses `query()` + `continue: true` (§23.1). |
| Message stream | Five types: `SystemMessage(init)` → `Assistant`/`User` per turn → `ResultMessage`. Iterate to completion; branch on `result.subtype` (§23.1, §23.3). |
| Sessions | Persisted as JSONL under the encoded `cwd`; capture `session_id`, then **resume** (by id), **continue** (most recent), or **fork** (`resume`+`fork_session`). Mismatched `cwd` silently breaks resume (§23.2). |
...
| Stopping | Primary exit = model finishes (no tool calls); `max_turns`/`max_budget_usd` are *secondary* guardrails surfacing as `error_*` subtypes you resume from (§23.3). |
| Control surface | Precedence: hooks → deny rules → ask rules → mode → allow rules → `canUseTool`. Hard rules go in hooks (`deny` wins, runs first); `bypassPermissions` ignores `allowed_tools` (§23.4). |
| Tools & MCP | In-process `@tool` + `create_sdk_mcp_server` (fast, secrets stay local) vs external `mcp_servers` (reusable, language-agnostic). MCP tools are `mcp__server__action`; watch schema context cost (§23.5). |

> **Beyond the exam, built on it.** Chapter 3 gives you the agent concepts the exam tests; this chapter is those concepts as the SDK actually implements them — the loop is `query()`, the hooks are real callbacks with a precedence order, the context management is automatic compaction, and "least privilege" is a six-step rule chain. Master this and you can ship an SDK agent that is cheap, safe, observable, and resumable — then graduate to **Managed Agents** (Chapter 24) when you want Anthropic to run the loop and the sandbox for you.
...
flowchart TD
    ...
    Client -. more control, more glue code .-> SDK
    SDK -. less ops, less control .-> Managed
```

| Runtime | Who runs the loop | Who owns compute | You write | You operate |
|---|---|---|---|---|
| **Client SDK** (`anthropic`) | You | You | The `while` on `stop_reason`, every tool, retries, context trimming | Everything |
...
> ⚠️ **The #1 rule (memorize):** `model` / `system` / `tools` live on the **agent**, **never** on the session. The session only takes a pointer. Calling `agents.create()` on every request is the classic anti-pattern — it churns versions, fragments your observability (every run looks like a different agent), and defeats the whole point of a *persisted, versioned* definition.
...
**The one sanctioned exception — per-session overrides (beta).** You *can* now change parts of the config for a single session without versioning the agent: pass `agent` as `{type: "agent_with_overrides", id, …}` and include any of `model`, `system`, `tools`, `mcp_servers`, or `skills`. Use it for one-off experiments — try a cheaper model or grant one extra tool for a single run — **not** as a back door for per-request data. The rules are strict and worth memorizing: an override **replaces** the field in full (it never merges — a `tools` override must list *every* tool the session should have), **omitting** a field inherits it from the referenced version, and `null` / `[]` **clears** it — with two carve-outs: `model` is never clearable (`model: null` → `400 agent_model_required`), and `tools` cannot be cleared while the effective `skills` is non-empty, because skills require the `read` tool. Crucially, overrides **do not** create a new version or mutate the agent resource, and the response's `agent` still carries the base `id` / `version` — so the "which definition ran this?" traceability from payoff #2 survives, and other sessions on the same agent are unaffected. Source: [Override agent configuration for a session](https://platform.claude.com/docs/en/managed-agents/sessions).
...
**Live token preview — event deltas (opt-in).** By default `agent.message` is **buffered**: each event arrives only *after* the model request that produced it finishes. To render text as it is generated, opt in **per connection** by adding the `event_deltas[]` query parameter to `GET /v1/sessions/{id}/events/stream` (accepted values `agent.message` and `agent.thinking`; any other value is a 400, and only the session-level stream accepts it — [multi-agent thread](https://platform.claude.com/docs/en/managed-agents/multi-agent) streams reject it). A previewed message emits one `event_start` (announcing the upcoming event's `id`) followed by `event_delta` events carrying incremental text, keyed by `event_id` + `delta.index`. **The preview is a display aid, not the record** — the buffered `agent.message` stays authoritative, a client that ignores previews still receives a complete stream, and (unlike [Messages-API streaming](https://platform.claude.com/docs/en/build-with-claude/streaming)) there are no per-block start/stop events and the delta type is `content_delta`, so Messages-API accumulator code does **not** carry over unchanged. An `agent.thinking` preview emits only the `event_start`; its content still arrives in the buffered event. Source: [Event deltas](https://platform.claude.com/docs/en/managed-agents/events-and-streaming#event-deltas).
...
> **Trade-off in one line:** fully managed minimizes ops; self-hosted maximizes control over *where tool side effects happen*. Pick self-hosted only when a real constraint (residency, reachability, licensing, audit) forces it — not by default.
...
> **Pitfall:** assuming "managed" means "fully observable." You get rich *event-level* visibility, but the hosted runtime is a black box at the OS level by design. Match your audit requirements to the hosting choice **before** you build.
...
def review_pull_request(pr_ref: str):
    return client.beta.sessions.create(
        agent={"type": "agent", "id": agent.id, "version": agent.version},
        environment_id=environment.id,
        input={"pr": pr_ref},          # per-run data goes in the SESSION, not a new agent
    )
```
...
| Concept | What to remember |
|---|---|
| Three runtimes, one axis | Client SDK (you run the loop) → Agent SDK (library runs it in your process) → Managed Agents (Anthropic runs the loop **and** the sandbox). Choose by *who runs the loop and who owns the compute* (§24.1). |
| Object model | **Agent** = persisted, versioned config (`model`/`system`/`tools`); **Session** = one run that only *points* at the agent; **Environment** = reusable container template (§24.2). |
| The #1 rule | `model`/`system`/`tools` live on the **agent**, never the session. Create the agent **once**, reference by ID; `agents.create()` per run is the classic anti-pattern (§24.2). |
...
| Hosting topology | Fully managed (Anthropic's container, zero ops) vs **self-hosted sandbox** (`type: self_hosted`, your infra via outbound-polling worker). Anthropic runs the loop either way; choose self-hosted only when residency/reachability/licensing/audit forces it (§24.4). |
...
> *Reference — extends Chapter 4.* Docs: [Model Context Protocol](https://modelcontextprotocol.io/) · [Specification](https://modelcontextprotocol.io/specification) · [Authorization](https://modelcontextprotocol.io/specification/draft/basic/authorization) · [MCP connector](https://platform.claude.com/docs/en/agents-and-tools/mcp-connector).
...
We deliberately do **not** repeat Chapter 4. Instead we go one layer down on each of its topics, and add two it never covered — **OAuth 2.1 authorization for remote servers** and **sampling** (the server asking the *client's* model to think). By the end you should be able to design a production-grade remote MCP server and defend its transport, auth, and security choices.
...
§25.6 then builds a complete remote **Knowledge-Base MCP server** — OAuth, a resource, a sampling-backed `summarize` tool, and a prompt — §25.7 previews the next spec revision, and §25.8 distills the engineering rules.
...
- **Sessions.** On initialize the server may return an `Mcp-Session-Id` header; the client echoes it on every later request. This is how a *stateful* server (one holding per-client subscriptions or sampling state) ties requests together — and how it can expire a session.
...
```mermaid
flowchart TD
    Start[Client to remote server<br/>POST initialize] --> Sess[Server returns<br/>Mcp-Session-Id]
    ...
    SSE -. drop and reconnect .-> Resume[Resume with Last-Event-ID<br/>server replays missed events]
```
...
**Reaching a server that lives inside your network — MCP tunnels.** Streamable HTTP assumes the server is reachable at a public URL. When the MCP server sits inside a private network, the enterprise-safe pattern is **MCP tunnels** (research preview): a tunnel stack you run inside your network opens an **outbound-only** connection, so Claude can reach the server **without opening inbound firewall ports, exposing it to the public internet, or allowlisting Anthropic's IP ranges**. Each private server gets a hostname under your tunnel domain, which you pass to Managed Agents or the Messages API's MCP connector exactly like any other remote server URL — only the `url` is tunnel-specific. Three independent layers protect every request: outer **mTLS** (with IP validation) to the transport edge, **inner TLS** terminated by a certificate only you hold — so the transport provider cannot read payloads — and each server's own **OAuth**. Management runs through the Tunnels API under the `anthropic-beta: mcp-tunnels-2026-06-22` header and the `workspace:manage_tunnels` WIF scope. Source: [MCP tunnels](https://platform.claude.com/docs/en/agents-and-tools/mcp-tunnels/overview).
...
**Rate-limit and bound cost.** A remote server is callable in a loop. Rate-limit per session, cap result sizes (an unbounded `search` result can blow the context window), and put timeouts on every downstream call so one slow dependency can't hang the agent.
...
## 25.7 The Next Revision — the 2026-07-28 Spec (Preview)

Everything above is the **current stable spec, `2025-11-25`** — what the exam and today's SDKs assume. But the protocol is about to take its largest step since launch. The **`2026-07-28` revision** is locked as a Release Candidate (frozen 2026-05-21, final publication **2026-07-28**), with beta SDKs already shipping. You will not be tested on it, but an architect standing up MCP infrastructure in H2 2026 has to know where the ground is moving. Four changes matter.

**A stateless protocol core.** The `initialize` handshake and the per-connection session model are *gone*. Instead of exchanging protocol version, client info, and capabilities once at connect time, every request now carries them in `_meta` fields. The payoff is deployment, not features: **any request can land on any server instance**, so the sticky routing and shared session store that §25.2's `Mcp-Session-Id` forced on you dissolve at the protocol layer. A remote MCP server becomes an ordinary horizontally-scaled HTTP service — the same architectural win the Messages API already has (§1.2). (A stateful session model still exists for servers that genuinely need affinity; it is simply no longer the default the whole protocol is built around.)

**An Extensions framework.** Optional capabilities move *out* of the core spec into independently-versioned **extensions**, each identified by a reverse-DNS id, living in its own `ext-*` repository with delegated maintainers. Client and server negotiate them through an `extensions` map at discovery — opt-in, à la carte. The core stays small and stable while the ecosystem evolves on its own clock — the structural fix for "every useful idea bloats the base protocol."

**Tasks and MCP Apps ride that framework.** **Tasks** (long-running work) graduates from an experimental core feature to an extension with a cleaner lifecycle: the server *creates* the task and the client *drives* it via `tasks/get` / `tasks/update` / `tasks/cancel` — the standard answer to "this tool call takes ten minutes." **MCP Apps** lets a server ship an interactive HTML UI rendered in a **sandboxed iframe** that talks back over JSON-RPC with a full audit trail — server-rendered UI without handing the page arbitrary reach.

**Authorization hardened toward OAuth/OIDC.** The OAuth 2.1 story of §25.3 tightens: mandatory `iss` validation (**RFC 9207**, closing an authorization-code-injection gap), an `application_type` declaration in Dynamic Client Registration, and W3C **Trace Context** propagation in `_meta` (SEP-414) so one trace follows a call host → client → server → downstream. Paired with it is the spec's first **formal deprecation policy**: features move Active → Deprecated → Removed with a **minimum 12-month window** at each stage — so the "SSE is deprecated" churn of §25.2 now runs on predictable rules.

**Pitfalls.**
- Treating the RC as shippable production truth. Until **2026-07-28** the authoritative spec is `2025-11-25`; build to it, and pin your SDK — a beta SDK tracking the RC can change under you.
- Assuming the stateless core *forbids* sessions. It doesn't; it demotes them from default to opt-in, so a server that truly needs affinity can still ask for it.
- Reaching for an extension's id or shape from memory. Extensions version independently of the core — confirm the reverse-DNS id and its current version against its `ext-*` repo, not the core changelog.

## 25.8 Engineering Focus — Key Takeaways

| Concept | What production engineering requires |
|---|---|
| Control model | Tools = model-controlled (act), Resources = app-controlled (inform), Prompts = user-controlled (templates); add `annotations` + `outputSchema` to tools (§25.1). |
...
| stdio mechanics | `stdout` is protocol-only — **all logs to `stderr`**; the host owns the lifecycle; init must be fast and non-blocking (§25.2). |
| HTTP sessions | `Mcp-Session-Id` ties a stateful session; SSE event ids enable **resumability**; legacy two-endpoint SSE is deprecated; mind load-balancer affinity (§25.2). |
...
| Discovery at scale | `mcp__<server>__<tool>` namespacing; **MCP tool search** loads tools lazily; keep tool lists small — tool count is a budget (§25.5). |
| Spec direction | `2025-11-25` is current; the `2026-07-28` RC makes the core **stateless** (any request → any instance), moves optional features to independently-versioned **Extensions** (Tasks, MCP Apps), and hardens OAuth (RFC 9207 `iss`; a 12-month deprecation policy) (§25.7). |
...
> *Reference — beyond the exam scope.* Docs: [Claude Code docs](https://code.claude.com/docs/en/overview) · [What's new](https://code.claude.com/docs/en/whats-new) · [Subagents](https://code.claude.com/docs/en/sub-agents) · [Plugins](https://code.claude.com/docs/en/plugins) · [Headless](https://code.claude.com/docs/en/headless).
...
**`isolation: worktree`** runs a subagent's file edits inside a **throwaway git worktree** — a separate checkout — so its changes never touch your working tree until you merge them. This is the blast-radius control for *speculative* edits: let a subagent attempt a risky refactor in an isolated worktree, inspect the diff, and keep it only if it's good. If it's bad, the worktree is discarded and your tree is pristine.
...
flowchart TD
    ...
    D -->|Risky speculative edits| WT[isolation worktree mode<br/>edits in a throwaway checkout]
...
|---|---|---|---|---|
| `CLAUDE.md` | Humans | Project / user / dir | Project level: yes | The team contract — standards, conventions |
| `MEMORY.md` | Claude (auto) | Machine-local | No | Claude's own accumulated learnings |
...
**Headless / `-p` for CI.** `claude -p "<task>"` (a.k.a. `--print`) runs non-interactively and exits — the foundation for CI jobs, pre-commit checks, and batch pipelines. For machine-readable output: `--output-format json`, and **`--json-schema`** to force a **schema-conforming `structured_output`** so a downstream step can parse the result deterministically. **`--bare`** skips auto-discovery (CLAUDE.md, settings, plugins) for a **deterministic, reproducible run** — the same inputs yield the same behavior, which is what CI needs; it is slated to become the `-p` default. (This is the same headless surface Chapter 5 introduced for CI; here it pairs with the orchestration and permission machinery below.)

**Permission modes — the safety dial.** The mode decides what executes without a human prompt:

| Mode | Behavior | Typical use |
|---|---|---|
| `default` (shown as **Manual** since July 2026) | Ask before sensitive actions | Interactive development |
| `acceptEdits` | Auto-accept file edits, still gate commands | Trusted local editing |
| `plan` | Read/investigate only — **no mutations** | Planning before approval |
| `bypassPermissions` | No prompts at all | Sandboxed, fully-trusted automation only |
| `auto` | A **server-side safety classifier** decides per action | Smart unattended runs |
| `dontAsk` | Only **read-only / pre-approved** operations proceed | Conservative automation |
...
| Orchestration spectrum | Match the mode to context economics: subagents (model holds the plan) → teams (a lead + task list) → dynamic workflows/`ultracode` (a script; only final answers return) (§26.2). |
| Fork vs fresh vs worktree | Fresh + explicit context for parallel fan-out; `context: fork` for deep continuation; `isolation: worktree` for anything you might discard (§26.3). |
| Checkpoints are not Git | `/rewind` undoes Claude's *edits* only — it does **not** track bash/external file changes; keep using Git (§26.4). |
| Memory layers | `CLAUDE.md` = team contract (shared); `MEMORY.md` = Claude's own machine-local notebook (not shared) (§26.4). |
| Plugins + expanded hooks | Plugins ship Skills/agents/hooks/MCP as one versioned unit; `SubagentStart`/`PreCompact`/`SessionEnd` add fleet-scale observability and policy (§26.5). |
...
| Unattended safety | Headless `-p` + `--json-schema` + `--bare` for deterministic CI; prefer the `auto` classifier or `dontAsk` over `bypassPermissions`, and back rules with hooks (§26.6). |
...
def grade_outcome(run) -> dict:
    return {
        ...
        "no_requests": "import requests" not in run.diff_added_lines(),  # migration actually happened
    }
```
...
def grade_pr_text(pr_text, diff) -> JudgeVerdict:
    return judge(
        model="claude-sonnet-5",          # NOT the model that wrote the PR -> avoids self-enhancement bias
        rubric=PR_RUBRIC,                    # faithful / complete / flags-risk, each pass/fail
...
def passed(task) -> bool:
    trials = [run_agent(task) for _ in range(K)]            # K trials -> non-determinism
    def ok(r):
...
- **Judge-calibration test:** on a human-labeled sample of PR descriptions, assert the judge agrees with humans ≥ 90%; if not, the rubric — not the human label — is wrong (§27.3).
...
| Trajectory vs final answer | Grade the path *and* the outcome; assert path *properties*, not a verbatim tool sequence (§27.2). |
| LLM-as-judge | Concrete rubric, reasoning-before-verdict, a *different* model than the agent, and bias mitigation; validate against human labels before trusting it (§27.3). |
...
| Metrics + CI gating | Agents are non-deterministic — run k trials, report **pass@k**, and gate CI on outcome *and* trajectory *and* against a baseline, not just an absolute floor (§27.5). |
...
> **Sonnet 5 is the current balanced tier**, succeeding Sonnet 4.6 (now a legacy model that is still callable via `claude-sonnet-4-6`). Sonnet 5 carries **introductory pricing of $2 / $10 per MTok through 2026-08-31**, after which standard **$3 / $15** applies. On Claude Sonnet 5 the `effort` parameter defaults to `high` on the Claude API and Claude Code. Source: [Models overview](https://platform.claude.com/docs/en/about-claude/models/overview), [Pricing](https://platform.claude.com/docs/en/about-claude/pricing).
>
> **Migrating from Sonnet 4.6 to Sonnet 5** — three API behaviors changed: [adaptive thinking](https://platform.claude.com/docs/en/build-with-claude/adaptive-thinking) is now **on by default**; manual extended thinking (`thinking: {type: "enabled", budget_tokens: N}`) is **removed and returns a 400 error**; and setting the sampling parameters `temperature` / `top_p` / `top_k` to non-default values **returns a 400 error**. [Priority Tier](https://platform.claude.com/docs/en/api/service-tiers) is **not available** on Sonnet 5 (otherwise it carries the same tools and platform features as Sonnet 4.6, with a 1M context window and 128K max output). Sonnet 5 also uses a **new tokenizer that produces roughly 30% more tokens for the same text**, so re-measure prompt size and cost with the [token counting API](https://platform.claude.com/docs/en/build-with-claude/token-counting) (`model="claude-sonnet-5"`) instead of reusing Sonnet 4.6 estimates. Source: [What's new in Claude Sonnet 5](https://platform.claude.com/docs/en/about-claude/models/whats-new-sonnet-5).
...
```mermaid
flowchart TD
    A[Incoming task] --> B{Complex reasoning<br/>or long-horizon agentic?}
    B -->|no, well specified| C{Latency critical<br/>or very high volume?}
    C -->|yes| D[Haiku 4.5<br/>fast and cheap]
    C -->|no| E[Sonnet 5<br/>balanced default]
    B -->|yes| F{Does Sonnet pass<br/>your eval set?}
    F -->|yes| E
    F -->|no| G[Opus 4.8<br/>flagship default]
    G --> H{Still failing on the<br/>hardest cases?}
    H -->|yes| I[Fable 5<br/>absolute ceiling]
    H -->|no| G
```
...
MODELS = {
    ...
    "subagent":    "claude-haiku-4-5",   # narrow fetch/extract under the coordinator
...
def route(message) -> str:
    result = classify(message, model=MODELS["triage"])   # Haiku: category + confidence
    if result.category == "faq":
        return "faq"
    if result.complex or result.high_stakes or result.confidence < 0.7:
        return "coordinator"          # bias to Opus when unsure — cost of a wrong cheap answer is higher
    return "standard"
```

Cache the stable prefix on the high-volume Sonnet path so the reused system prompt and policy text are billed at ~0.1× after the first call:
...
resolve(
    ...
    system=[{"type": "text", "text": POLICY_PREAMBLE,
             "cache_control": {"type": "ephemeral"}}],   # stable prefix -> cache read ~0.1x
)
```
...
| Default to Sonnet | Sonnet 5 is the production default; move *down* to Haiku for simple/fast/high-volume, *up* to Opus only when quality demands it (§28.2). |
...
| Cost levers before downgrades | Caching (~0.1× reads), Batch API (50% off), tight `max_tokens` cut cost *without* touching quality — do these first (§28.4). |
...
> **Beyond the foundations exam.** The exam won't ask you to compare models, but production will — every day, on every request. Master the ladder, the fail-upward router, and the tiered coordinator, and you turn model choice from a guess into an engineered, measurable, and continuously optimizable part of the system.

# Chapter 29: Deployment & Enterprise Integration

> *Reference — beyond the exam.* Docs: [platform availability](https://platform.claude.com/docs) · [Workload Identity Federation](https://platform.claude.com/docs/en/manage-claude/workload-identity-federation) · [Enterprise-Managed Authorization](https://modelcontextprotocol.io/extensions/auth/enterprise-managed-authorization)

Everything before this point is about what happens *inside* the agent — the loop, the tools, the context. Production asks two questions the exam never does: **where does this run**, and **who is it when it runs there?** Which cloud surface serves the model, how a fleet of unattended workloads authenticates without a drawer full of API keys, who decides which MCP servers an organization's agents may touch, and where credentials live so a prompt injection cannot steal them. The answers exist in this guide — a parity table in the appendix, a WIF row, an EMA bullet in §25.3, vaults in Chapter 24 — but scattered. This chapter assembles them into one decision framework.

By the end you should be able to:

- **Choose a deployment surface** (§29.1) — first-party API, Claude Platform on AWS, Bedrock, Vertex, or Foundry, decided by feature parity and compliance, not habit.
- **Retire static API keys** (§29.2) — replace `sk-ant-…` secrets with Workload Identity Federation for every unattended workload.
- **Govern MCP org-wide** (§29.3) — pin the approved server catalog with managed MCP and centralize authorization with EMA.
- **Keep secrets out of the context window** (§29.4) — egress injection, so the model can never leak what it never held.

§29.5 assembles these into a reference enterprise architecture, and §29.6 distills the rules.

## 29.1 Choosing a Deployment Surface

Five surfaces serve Claude, and they are **not interchangeable** — the differences are architectural, not cosmetic. The full comparison lives in the appendix's cloud-provider table; what matters here is the decision procedure:

```mermaid
flowchart TD
    A[Where must Claude run?] --> B{Architecture needs server tools,<br/>Batches, Files API,<br/>or Managed Agents?}
    B -->|yes| C{Billing / procurement?}
    C -->|Anthropic direct| D[First-party API]
    C -->|AWS Marketplace + IAM| E[Claude Platform on AWS<br/>same-day full parity, SigV4]
    B -->|no — inference + tool use is enough| F{Compliance or cloud commitment?}
    F -->|FedRAMP High / DoD IL4-5| G[Amazon Bedrock]
    F -->|GCP estate| H[Google Vertex AI]
    F -->|Azure estate| I[Microsoft Foundry<br/>most features beta]
```

The load-bearing fact: **the first-party API and Claude Platform on AWS are full-surface; Bedrock and Vertex are model-inference + tool-use.** No Batches, no Files API, no code execution, no web fetch, no Managed Agents on Bedrock/Vertex — you bring your own loop and your own infrastructure around a bare model. That is a fine trade when FedRAMP High or a cloud commitment requires it, but it must be made *before* you architect, not after.

**The classic failure** is designing on first-party features — a Batches-based nightly pipeline, Files-API document reuse, a Managed Agents fleet — and then discovering during "the Bedrock migration" that none of those surfaces exist there. Porting is then a re-architecture. Check the [availability table](https://platform.claude.com/docs) first, and treat "which surface" as a requirements decision with a named owner, exactly like a database choice.

## 29.2 Workload Identity: Retiring the Static API Key

A static `sk-ant-…` key is fine on a laptop and a liability in a fleet. At scale it has four structural problems: someone must **mint and rotate** it; it can **leak** (logs, images, repos) and keeps working for whoever holds it; every caller sharing it looks like the **same identity**, so you can't attribute usage or scope permissions per workload; and revocation is **all-or-nothing** — killing the key kills every consumer at once.

**Workload Identity Federation (WIF)** removes the standing secret entirely. The workload proves who it is with a short-lived OIDC JWT issued by its *own* platform — AWS, GCP, Microsoft Entra ID, GitHub Actions, Kubernetes, SPIFFE, or Okta — and exchanges it at `POST /v1/oauth/token` for a scoped, minutes-long Anthropic token that acts as a **service account**:

```mermaid
sequenceDiagram
    participant W as Workload (CI job / service)
    participant IdP as Its identity provider (OIDC)
    participant T as Anthropic /v1/oauth/token
    participant API as Claude API
    W->>IdP: request identity token
    IdP-->>W: short-lived OIDC JWT
    W->>T: exchange JWT for an access token
    T-->>W: scoped Anthropic token (minutes-long)
    W->>API: normal API calls with that token
```

```python
# 1. The platform the workload runs on issues its identity (no secret stored anywhere).
jwt = ci_platform.request_oidc_token(audience="https://api.anthropic.com")

# 2. Exchange it for a scoped Anthropic token — minutes-long, service-account semantics.
#    (Exact exchange parameters: see the WIF docs.)
token = exchange_at_oauth_endpoint(jwt)

# 3. Use it exactly like an API key — every endpoint, the SDKs, and Claude Code accept it.
client = Anthropic(auth_token=token)
```

What this buys, structurally: **nothing to mint, rotate, or leak** — a captured token dies in minutes; **identity-bound access** — permissions and revocation live in *your* IdP (remove the workload's role and its access is gone everywhere, instantly); and **per-workload attribution** — usage, limits, and audit trails per service account instead of one anonymous shared key.

**Pitfalls.** Tokens expire in minutes by design — refresh before TTL rather than caching one for the job's lifetime. Humans on laptops still use API keys; keep those scoped and low-privilege rather than pretending WIF covers them. Never log the exchanged token. And resist the "fallback static key just in case" — it silently reintroduces the exact standing secret WIF exists to eliminate.

## 29.3 Governing MCP at Enterprise Scale

An organization running agents against MCP has two distinct governance questions, and they have two distinct mechanisms:

**Which servers exist at all — the catalog.** **Managed MCP** (§26.5) lets the org pin an approved server set via `managed-mcp.json`, distributed like any other configuration (a plugin is the natural vehicle — §26.5). Developers don't assemble their own server lists; the platform team ships one.

**Who may use them — the authorization.** **Enterprise-Managed Authorization (EMA)** — a *stable* MCP extension since June 18, 2026 — moves MCP authorization into the corporate identity provider. An admin enables a server for the organization once; users and workloads then get access according to their existing IdP groups and roles. No per-user OAuth consent screens, and revocation is central and immediate: remove someone from the IdP group, and their access to that server disappears everywhere. Okta is the launch IdP; Asana, Atlassian, Canva, Figma, Linear, and Supabase are among the first supported servers.

Contrast the mechanisms you might be tempted to use instead:

| Mechanism | What it actually is | Why it fails as governance |
|---|---|---|
| Server list in CLAUDE.md | Prompt text | Advisory — the model *usually* complies; nothing enforces it |
| Firewall blocks | Network control | Stops traffic, but grants nothing, knows no identities, manages no consent lifecycle |
| Per-user OAuth consents | Real authorization | Doesn't scale, no central view, revocation is per-user-per-server archaeology |
| **managed MCP + EMA** | **Configuration + identity** | Deterministic catalog, group-based access, instant central revocation, audit trail |

This is the Chapter 13 principle wearing enterprise clothes: **when a prohibition matters, it lives in configuration and identity, not in wording.**

## 29.4 Secrets at Runtime: Egress Injection

Chapter 24 introduced the pattern in the Managed Agents context; it deserves stating as a general law: **the model's context window must never contain a credential.** Anything in context can be emitted — coaxed out by a prompt-injected PR, echoed into a log, summarized into a transcript. The fix is structural, not behavioral:

```mermaid
flowchart LR
    M[Model context<br/>never holds secrets] --> T[Tool call]
    T --> G[Egress boundary<br/>vault injects the credential]
    G --> S[(External service)]
```

- **Vaults (Managed Agents):** the secret is substituted only at egress — an agent told to `echo $GITHUB_TOKEN` prints nothing useful, because the token was never in its world (§24.5).
- **MCP servers hold their own credentials:** the server authenticates to its backend service server-side; the model sees tools, never keys (§4.2 — the same reason Q82's "API key in CLAUDE.md" is wrong).
- **Self-hosted loops:** credentials live in the *tool process's* environment, injected by your runtime, never interpolated into prompts or tool results.
- **WIF (§29.2)** shrinks even the platform credential to a minutes-long token bound to an identity you control.

**The test is mechanical:** run a prompt-injection drill that tries to print every credential the agent could plausibly reach, and assert the transcript contains none of them. If a secret can appear in a transcript, it will.

## 29.5 The Reference Enterprise Architecture

Assembled, the pieces look like this — a nightly security-review fleet as the running example:
...
flowchart TD
    ...
    IdP -- "WIF: JWT → minutes-long token" --> Run[Agent workload<br/>claude -p / Agent SDK / Managed Agents]
    IdP -- "EMA: group-based authorization" --> MCP[Approved MCP servers<br/>pinned via managed-mcp.json]
    Run --> MCP
    Run --> API[Claude API<br/>surface chosen in §29.1]
    MCP -- "vault / egress injection" --> Ext[(Internal systems)]
    Run -- "PreToolUse deny rules + hooks" --> Guard[Deterministic guardrails]
    Run -- "SessionEnd audit hook" --> SIEM[(Audit trail / SIEM)]
```

The nightly job starts on a schedule; the runner authenticates via **WIF** (no static key exists anywhere in CI); it may talk only to the MCP servers the org pinned, and only because its service account sits in the right **EMA** group; the GitHub token those servers need is **vault-injected at egress**, so the reviewed PRs can't phish it; "never push to main" is a **deny rule and a `PreToolUse` hook** (§13.4, §26.7), not a sentence in a prompt; and a **`SessionEnd` hook** ships the audit trail. The model surface is whichever §29.1 selected — the architecture above doesn't change.

### Validation

- **Key-inventory test:** grep the CI configuration and secret stores; assert zero `sk-ant-` keys remain for unattended workloads.
- **Revocation test:** remove the workload's IdP group membership; assert its next token exchange fails and its MCP access is gone.
- **Exfiltration drill:** submit a prompt-injected input that tries to echo every reachable credential; assert none appears in any transcript or log.
- **Parity test:** list the platform features your architecture calls (Batches? Files? code execution?) and assert every one exists on the surface you chose.

## 29.6 Engineering Focus — Key Takeaways

| Concept | What to remember |
|---|---|
| Surface choice | Parity decides: first-party + Claude Platform on AWS are full-surface; Bedrock/Vertex are inference + tool use — check the availability table **before** architecting (§29.1). |
| WIF | Exchange a short-lived OIDC JWT at `POST /v1/oauth/token` for a minutes-long scoped token — nothing to mint, rotate, or leak; revocation lives in your IdP (§29.2). |
| Catalog vs authorization | `managed-mcp.json` pins **which** servers exist; EMA decides **who** may use them — group-based, centrally revocable, audited (§29.3). |
| Secrets | Egress injection: the context window never holds a credential, so prompt injection can't steal what isn't there (§29.4). |
| The principle | Identity, authorization, and secrecy are **configuration, not prompt text** — the same law as deny rules and hooks (Ch 13, §26.6). |

> **Beyond the foundations exam.** The exam stops at the agent's edge; production starts there. Choose the surface by parity, make every workload prove who it is, govern MCP through identity, and keep secrets out of the model's world entirely — and the same agent you built in Parts I–III becomes something an enterprise can actually run.
...
- A) Order the prompt static-first and set a cache breakpoint after the preamble, so repeat requests read it from the prompt cache at ~0.1×. **[CORRECT]**
...
**Why C:** Forcing a verbatim supporting quote before each value grounds the extraction in retrieved text: a field with no quote surfaces as a gap instead of a confabulation, directly countering the lost-in-the-middle failure mode. "Read carefully" (A) is advisory. Output size (B) has nothing to do with mid-context attention. Temperature (D) changes variance, not grounding.
...
| Claude Mythos 5 | `claude-mythos-5` | 1M | 128K | $10 | $50 | Same as Fable 5 with dual-use safeguards lifted for approved organizations — Project Glasswing partners (cyber) plus a trusted-access program for biology; a systematic application route is planned. |
...
| Claude Sonnet 5 | `claude-sonnet-5` | 1M | 128K | $3 | $15 | Best speed/intelligence balance for high-volume production. Introductory $2 / $10 through 2026-08-31. |
...
- **Sonnet 5 migration specifics:** Sonnet 5 ships on the newer tokenizer (Opus 4.7+ / Fable 5 lineage) — the same text yields **~30% more tokens** than pre-4.7 models, so re-measure with `count_tokens` before estimating cost or fit. Adaptive thinking is on by default; non-default `temperature` / `top_p` / `top_k` and manual `budget_tokens` thinking both return **400**; and **Priority Tier** is not offered on Sonnet 5. Source: [Platform release notes — June 30, 2026](https://platform.claude.com/docs/en/release-notes/api).
...
**[Claude Cowork](https://www.anthropic.com/product/claude-cowork)** brings Claude Code's agentic engine to *knowledge work* — it runs inside **Claude Desktop** (no terminal) and completes multi-step tasks end-to-end for non-developers: researchers, analysts, ops, legal, and finance teams who live in documents, data, and files. Same architecture as Claude Code (gather context → act → verify, with sub-agent fan-out), repackaged for people who don't write code.

- **Local file access** — reads from and writes to your local files, folders, and desktop apps, synthesizing across sources without you coordinating each step.
- **Sub-agent coordination** — breaks a task into parallel workstreams, the desktop cousin of §26.2's orchestration.
- **Professional outputs** — Excel workbooks with working formulas, PowerPoint decks, and other artifacts, not just chat text.
- **Availability** — all paid plans, via the Claude desktop app. [Docs](https://claude.com/docs/cowork/overview).

> **Architect's takeaway:** the agentic loop is *not* coding-specific. Cowork is the same loop Claude Code runs, pointed at spreadsheets and reports instead of a repo — the clearest signal that "agent" is a general execution pattern, not a developer-only feature.

### 2e. Claude Tag

**[Claude Tag](https://www.anthropic.com/news/introducing-claude-tag)** is Claude as a **shared team member**, starting in **Slack**. Tag `@Claude` in a channel with a request in plain language; it breaks the work into stages, executes them with its connected tools, and replies in a thread with what it produced.

- **Multiplayer by design** — one Claude per channel that *everyone* shares: anyone can see what it is doing and pick up where a teammate left off (unlike a private 1:1 chat).
- **Memory & context** — remembers relevant information from the channels it is in and can plan future tasks.
- **Ambient mode** — when enabled, proactively surfaces information it thinks you will need from across its channels and connected tools.
- **Agent identity** — Tag rides the emerging *agent-identity access model*: the agent has its own team-wide identity and permissions rather than borrowing a person's. At Anthropic, ~65% of the product team's code now ships through their internal Claude Tag. [Docs](https://claude.com/docs/claude-tag/overview).

### 2f. Developer Platform / API
...
### 2g. Cloud providers (parity notes)
