...
Intro → Target Candidate → Exam Format → 5 Domains (weighted) → 8 Exam Scenarios → **PART I: Theory Foundations** (Chapters 1–13) → **PART II: Exam Domain Notes** → **PART III: Modern Agent Engineering** (Ch 14–16) → **PART IV: Platform Reference** (Ch 17–29) → **PART V: Architect–Professional (CCAR-P) prep** (Ch 30–31) → **PART VI: Developer/Associate blueprints** (Ch 32–33) → sample-question bank → **Practical Exercises** → **Appendices**. The English guide holds ~162 sample questions (12 in the examples section + 150 in the Practice Test: 121 CCAR-F across the 8 practice scenarios, plus per-cert groups — CCAR-P 10, CCDV-F 10, CCAO-F 9; `sync.mjs` tags each question with a `cert` field and the mock exam samples CCAR-F only). When editing one language, keep section ordering and headings parallel across languages — `web/scripts/sync.mjs` **fails the build** on section/question-count drift between en and zh-tw.
