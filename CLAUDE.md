...
- **The standalone quiz HTML is legacy for en/zh-tw.** The web site's question bank (built by `web/scripts/sync.mjs`) supersedes `practical_test_en.html` / `practical_test_zh-tw.html`; don't regenerate those two. The Python pipeline below still serves the non-site languages. Known inconsistency: `practical_test_ko.html` is committed but `ko` has no `LANG_TITLES` entry, so it can't be rebuilt as-is.
...
Intro → Target Candidate → Exam Format → 5 Domains (weighted) → 8 Exam Scenarios → **PART I: Theory Foundations** (Chapters 1–13) → **PART II: Exam Domain Notes** → **PART III: Modern Agent Engineering** (Ch 14–16) → **PART IV: Platform Reference** (Ch 17–29) → **PART V: Architect–Professional (CCAR-P) prep** (Ch 30–31) → **PART VI: Developer/Associate blueprints** (Ch 32–33) → sample-question bank → **Practical Exercises** → **Appendices**. The English guide holds ~317 sample questions (12 in the examples section + 305 in the Practice Test: 129 CCAR-F across the 8 practice scenarios, plus full-size per-cert banks — CCAR-P 63, CCDV-F 53, CCAO-F 60, matching each exam's official item count; `sync.mjs` tags each question with a `cert` field and the mock exam runs per-certification). When editing one language, keep section ordering and headings parallel across languages — `web/scripts/sync.mjs` **fails the build** on section/question-count drift between en and zh-tw.
